...
import { streamReviewChat } from '../api';

const QUICK_ACTIONS = [
  'What should I fix first?',
  'Explain the highest-severity issues',
  'Create a fix plan for all issues',
  'Which TODOs are most important?',
];

export default function ReviewChatPanel({ projectName, reviewData, onClose }) {
    ...
    <div className="mt-2 border border-white/5 rounded bg-white/[0.02] overflow-hidden">
      ...
      <div className="flex gap-1 p-1.5 overflow-x-auto">
        {QUICK_ACTIONS.map((q) => (
          <button key={q} onClick={() => send(q)} disabled={streaming}
            className="flex-shrink-0 text-[9px] px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 disabled:opacity-40">
            {q}
          </button>
        ))}
      </div>
      ...
      <div ref={scrollRef} className="max-h-48 overflow-y-auto space-y-1.5 px-2 pb-1.5">
        {messages.length === 0 && (
          <div className="text-[10px] text-gray-700 text-center py-3">
            Ask about the code review results...
          </div>
        )}
        {messages.map((m, i) => (
      ...
      <div className="flex gap-1.5 p-1.5 border-t border-white/5">
        ...
        <button onClick={() => send()} disabled={streaming || !input.trim()}
          className="px-1.5 py-1 rounded bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 disabled:opacity-40">
